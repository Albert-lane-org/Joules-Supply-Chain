...
export class SovereignProvenanceGuard {
  ...
  private checkHeader(content: string, ext: string, relPath: string): boolean {
    ...
    if (ext === '.json') {
      return trimmed.includes('0x3F8F9A1B2C3D') || trimmed.includes('#NoExploitRobot');
    }
    return trimmed.includes('PROVENANCE METADATA (.lvm / .lane v1.0)') &&
      ...
      trimmed.includes('#NoExploitRobot') &&
      trimmed.includes('#NoExploitAlbert');
  }
  ...
  private injectHeader(filePath: string, content: string, ext: string, relPath: string): boolean {
    try {
        ...
        try {
          ...
          if (typeof parsed === 'object' && parsed !== null) {
            parsed._provenance = {
              ...
              tags: LANE_CONSTANTS.HASHTAG_ARRAY,
              secAssertion: LANE_CONSTANTS.SEC_FILING_NO,
              policeReport: LANE_CONSTANTS.POLICE_REPORT_NO,
      ...
      if (['.ts', '.tsx', '.js', '.jsx', '.css', '.proto', '.rs'].includes(ext)) {
        ...
        // Skip formats that do not support raw header comments (e.g. PDF)
        return false;
      }

      // Clean old header before injecting new header to avoid duplicate headers
      const cleaned = content
        .replace(/^\/\* ==============================================================================\n \* PROVENANCE METADATA[\s\S]*?\* ============================================================================== \*\/\n\n?/, '')
        .replace(/^# ==============================================================================\n# PROVENANCE METADATA[\s\S]*?# ==============================================================================\n\n?/, '')
        .replace(/^<!-- ==============================================================================\n[\s\S]*?PROVENANCE METADATA[\s\S]*?============================================================================== -->\n\n?/, '');

      fs.writeFileSync(filePath, header + cleaned, 'utf-8');
      return true;
    } catch {
      return false;
    }
  ...
  private discoverFiles(dir: string): string[] {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        if (
          ['bun.lock', 'package-lock.json', 'yarn.lock'].includes(name) ||
          ['.pdf', '.lock', '.png', '.jpg', '.jpeg', '.gif', '.ico', '.bin', '.tar', '.gz', '.zip', '.woff', '.woff2', '.ttf'].includes(ext)
        ) {
          continue;
        }
