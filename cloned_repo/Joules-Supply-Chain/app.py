...
DEFAULT_WS_URI = "wss://froi.albertlane.net/FROI"
...
def extract_embedded_ipv4(ip_obj: ipaddress.IPv6Address) -> list[ipaddress.IPv4Address]:
    """Extracts all embedded IPv4 addresses from IPv6 transition mechanisms."""
    v4_addrs = []

    if getattr(ip_obj, "ipv4_mapped", None) and ip_obj.ipv4_mapped:
        v4_addrs.append(ip_obj.ipv4_mapped)

    if ip_obj in ipaddress.IPv6Network("::/96") and int(ip_obj) != 0:
        v4_int = int(ip_obj) & 0xFFFFFFFF
        v4_addrs.append(ipaddress.IPv4Address(v4_int))

    if ip_obj in ipaddress.IPv6Network("2002::/16"):
        v4_int = (int(ip_obj) >> 80) & 0xFFFFFFFF
        v4_addrs.append(ipaddress.IPv4Address(v4_int))

    if ip_obj in ipaddress.IPv6Network("64:ff9b::/96"):
        v4_int = int(ip_obj) & 0xFFFFFFFF
        v4_addrs.append(ipaddress.IPv4Address(v4_int))

    if ip_obj in ipaddress.IPv6Network("64:ff9b:1::/48"):
        v4_b1 = (int(ip_obj) >> 72) & 0xFF
        v4_b2 = (int(ip_obj) >> 48) & 0xFF
        v4_b3 = (int(ip_obj) >> 40) & 0xFF
        v4_b4 = (int(ip_obj) >> 32) & 0xFF
        v4_int = (v4_b1 << 24) | (v4_b2 << 16) | (v4_b3 << 8) | v4_b4
        v4_addrs.append(ipaddress.IPv4Address(v4_int))

    if ip_obj in ipaddress.IPv6Network("2001:0::/32"):
...
def is_ip_restricted(ip_obj: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    """Strictly validates IP address to prevent SSRF against internal networks."""
    ...
    return (
        ...
        or getattr(ip_obj, "is_benchmarking", False)
    )


def resolve_host_ip_sync(hostname: str) -> tuple[bool, str, str]:
    """Resolves hostname within cached worker pool and enforces global IP scope."""
...
def is_safe_endpoint(uri: str) -> tuple[bool, str, str]:
    """Validates URI syntax, length, scheme, and host safety."""
...
def render_live_feed():
    """Polls queue atomically and updates frame buffer inside Streamlit fragment."""
    while True:
        try:
    ...
    st.caption(f"Status: `{html.escape(st.session_state.ws_status)}`")
...
def run_ws_thread(target_uri: str, resolved_ip: str, msg_queue: queue.Queue, stop_evt: threading.Event, conn_id: str):
    """Worker function binding directly to pre-screened IP with TLS and host header pinning."""
